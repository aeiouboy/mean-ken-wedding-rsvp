(function() {
  ...
  var fragmentSource = [
    ...
    '  float edgeWarp = sin(fanRadius * 4.0 - u_time * 0.30) + sin(fanRadius * 7.3 + u_time * 0.23 + 1.7);',
    '  float wAngle = fanAngle + edgeWarp * 0.05;',
    '  float fanSlices = smoothstep(-0.05, 0.95, sin(wAngle * 14.0 + 2.0 + sway * 3.0));',
    '  float flickerA = sin(wAngle * 25.0 + 1.3) * sin(u_time * 1.25);',
    '  float flickerB = sin(wAngle * 13.0 + 4.1) * sin(u_time * 0.85 + 1.0);',
    '  float shimmer = 0.58 + 0.28 * flickerA + 0.16 * flickerB;',
    '  float stream = 0.62 + 0.38 * sin(fanRadius * 9.0 - u_time * 1.9);',
    '  float warpX = sin(fanPoint.y * 1.7 + u_time * 0.35) + sin(fanPoint.y * 0.9 - u_time * 0.21 + 2.0);',
    '  float warpY = sin(fanPoint.x * 1.3 - u_time * 0.28 + 1.0) + sin(fanPoint.x * 0.7 + u_time * 0.17);',
    '  float dapple = sin(fanPoint.x * 6.0 + warpX) * sin(fanPoint.y * 6.0 + warpY);',
    '  float lightPatch = smoothstep(-0.2, 0.7, dapple);',
    '  float shadowPatch = smoothstep(0.1, -0.75, dapple);',
    '  float beam = fanSlices * shimmer * stream * (0.55 + 0.45 * lightPatch);',
    ...
    '  color *= 1.0 - shadowPatch * fanMask * u_prism * 0.09;',
